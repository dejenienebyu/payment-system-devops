...
if [ -n "$EC2_IP" ] && [ "$EC2_IP" != "None" ]; then
    EC2_HEALTH=$(curl -s --max-time 5 http://$EC2_IP:3000/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['status'])" 2>/dev/null)
    echo "   EC2 API: ${EC2_HEALTH:-unreachable}"
fi

LB_URL=$(kubectl get service payment-api-service \
    -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null)
if [ -n "$LB_URL" ]; then
    LB_HEALTH=$(curl -s --max-time 5 http://$LB_URL/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['status'])" 2>/dev/null)
